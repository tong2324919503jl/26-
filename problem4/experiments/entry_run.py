...
from problem4.experiments.routing import EntryPolicy
from problem3.scenarios import generate_suite
from problem3.simulator import LocalSimulator,ObservationClient
count=int(sys.argv[1]);split=sys.argv[2] if len(sys.argv)>2 else 'development';rows=[];start=time.perf_counter()
for case in generate_suite(4,split,count):
 sim=LocalSimulator(case);sim.enter();error=None;result={}
 try:result=EntryPolicy().run(ObservationClient(sim))
