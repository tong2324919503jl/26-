...
def figure02():
    ...
    # Exact vertical sections of the four disks, including the two arc switches.
    # This panel shows C_safe itself; it does not impose the later 5 m exclusion.
    xmax=5*math.cos(E)+math.sqrt(1000**2-(5*math.sin(E))**2)
    half_chord=math.sqrt(1000**2-(995/2)**2)
    switch=(502.5*math.cos(E)+half_chord*math.sin(E),
            -502.5*math.sin(E)+half_chord*math.cos(E))
    xs=np.unique(np.r_[np.linspace(0.,xmax,7001),switch[0]])
    caps=np.array([np.sqrt(np.maximum(0.,1000**2-(xs-p[0])**2)) for p in centers])
    upper=np.min(np.array([p[1] for p in centers])[:,None]+caps,axis=0)
    lower=np.max(np.array([p[1] for p in centers])[:,None]-caps,axis=0)
    upper[[0,-1]]=0.; lower[[0,-1]]=0.
    assert np.allclose(upper,-lower,atol=1e-9) and np.all(upper>=lower)
    boundary=np.vstack([np.column_stack([xs,upper]),
                        np.column_stack([xs[::-1],lower[::-1]])])
    assert max(np.linalg.norm(boundary-np.array(p),axis=1).max() for p in centers)<=1000+1e-8
    fig,a=plt.subplots(figsize=(8.5,6.5))
    fig.subplots_adjust(left=.095,right=.98,bottom=.21,top=.975)
    poly(a,boundary,face='#e5e8ea',edge=INK,zorder=3)
    for i,p in enumerate(centers):
        col=BLUE if i<2 else GOLD
        a.add_patch(Circle(p,1000,fill=False,ec=col,lw=1.15,
                           ls='-' if i%2 else (0,(5,3)),alpha=.72 if i%2 else 1.,zorder=2))
        a.scatter(*p,s=15,color=col,edgecolors='white',linewidths=.4,zorder=7)
    # Label the near pair together without separating their true coordinates.
    a.annotate(r'$5u_\pm$',xy=centers[0],xytext=(-255,-185),
               color=BLUE,fontsize=11.5,ha='center',
               arrowprops=dict(arrowstyle='-',lw=.75,color=BLUE),zorder=8)
    a.annotate(r'$1000u_\pm$',xy=centers[2],xytext=(1285,-185),
               color=GOLD,fontsize=11.5,ha='center',
               arrowprops=dict(arrowstyle='-',lw=.75,color=GOLD),zorder=8)
    # One true radius in each color establishes the common 1000 m radius.
    for p,t,col in ((centers[1],150.,BLUE),(centers[2],30.,GOLD)):
        u=np.array([math.cos(math.radians(t)),math.sin(math.radians(t))])
        endpoint=np.array(p)+1000*u
        a.plot([p[0],endpoint[0]],[p[1],endpoint[1]],color=col,lw=.75,
               ls=(0,(2,3)),zorder=4)
        midpoint=np.array(p)+570*u
        a.annotate(r'$1000\,\mathrm{m}$',midpoint,xytext=(0,5),textcoords='offset points',
                   ha='center',rotation=-30 if t==150 else 30,rotation_mode='anchor',
                   color=col,fontsize=10,zorder=8)
        assert abs(distance(p,tuple(endpoint))-1000)<1e-9
    a.text(505,130,r'$\mathcal{C}_{\rm safe}$',ha='center',va='center',fontsize=20,zorder=6)
    a.text(505,-35,'保证收信区域',ha='center',va='center',fontsize=10.5,color=GREY,zorder=6)
    a.set(xlim=(-1120,2120),ylim=(-1120,1120)); axes_style(a)
    a.set_xticks([-1000,-500,0,500,1000,1500,2000])
    a.set_yticks([-1000,-500,0,500,1000])
    a.grid(color='#e8e8e8',linewidth=.45)
    handles=[]
    for d,col in ((5,BLUE),(1000,GOLD)):
        for sign,ls in (('+','-'),('-', (0,(5,3)))):
            handles.append(Line2D([0],[0],color=col,lw=1.15,ls=ls,
                                  label=fr'$B({d}u_{sign},1000)$'))
    fig.legend(handles=handles,loc='lower center',bbox_to_anchor=(.535,.032),
               ncol=2,frameon=False,columnspacing=3.4,handlelength=3.,
               handletextpad=.8,labelspacing=.7,fontsize=10)
    save(fig,'p2_02_four_disks','四圆盘及其保证收信交集',
         '以5u±与1000u±为圆心、1000米为共同半径的四个圆盘，其交集给出保证收信区域。'
         '蓝色和金色区分两组径向端点，实线与虚线分别对应正、负角端点。',
         '5.2.2 四圆盘安全候选域',SOURCES+['paper/plot_p2_intro_candidates.py'],
         notes='仅保留原图(a)，改为单幅等比例图；角误差仍为±1°，近端两圆心在当前尺度下近乎重合。'
               '交集用四圆盘的解析纵截面绘制，明确纳入上下换弧角点及左右端点，逐点核验包含。'
               '本图仅表示C_safe，未叠加5米正常测向排除条件或目标圆约束。',
         recommendation='用于5.2.2四圆盘公式及安全性证明。',
         data={'circle_centers_m':centers,'radius_m':1000,'near_center_separation_m':distance(*centers[:2]),
               'far_center_separation_m':distance(*centers[2:]),'boundary_points_verified':len(boundary),
               'boundary_method':'exact vertical circle sections with arc switch points',
               'x_support_m':[0.,xmax],'upper_arc_switch_m':switch,
               'lower_arc_switch_m':[switch[0],-switch[1]],'equal_aspect':True,'panel_count':1})
