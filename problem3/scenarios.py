...
def generate_case(problem: int, index: int = 0, split: str = 'development') -> dict:
    ...
    for j, ch in enumerate(channels):
        ...
        if family in ('boundary', 'outward'):
            ...
            r = 1800 if split == 'stress' else rng.uniform(1690, 1800)
            x, y = r*math.cos(a), r*math.sin(a)
        ...
        if family in ('short_radius', 'outward', 'boundary') or split == 'stress':
            radius = 1000 if split == 'stress' else rng.uniform(1000, 1030)
        orient = None
