...
from problem4.experiments.routing import LegacyPolicy,DetectedStopPolicy,InformationPolicy
...
for name,base in [('loc_legacy',LegacyPolicy),('loc_stop',DetectedStopPolicy),('loc_info',InformationPolicy)]:
 if len(sys.argv)>3 and name not in sys.argv[3:]:continue
 cls=type('LocRoute',(LocalizationMixin,base),{'localization_quantile':.2,'localization_width_fraction':.04,'localization_width_min':15.,'localization_enforce_range':True,'localization_history':True,'localization_history_slices':12,'info_weight':1.})
...
(Path(__file__).resolve().parents[1]/'results'/'iterations_v2').joinpath(f'combo_{split}_{count}.json').write_text(json.dumps(body,indent=2)+'\n',encoding='utf-8')
