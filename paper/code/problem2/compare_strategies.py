...
def sample_strategy(first: tuple, first_bearing: float, q: tuple,
    ...
    candidate = solve.prepare_candidate_region_local(polygon, q, circle_sides=360)
    for ir in range(61):
        r = 6+(1500-6)*ir/60
        for ia in range(21):
            ...
            for error in (-1., -.5, 0., .5, 1.):
                ...
                posterior = solve.clip_candidate_observation_local(candidate, theta)
                if not posterior:
...
def run_comparison(output_dir: Path, include_extended: bool = True) -> dict:
    ...
    for name in ("synthetic_case", "boundary_case"):
        ...
        for key, label, point in points:
            # 比较报告需要完整 B；短路的部分最大值不能充作完整上界。
            row = compact(solve._candidate_row(point, first, theta, evaluator, early_stop=False))
    ...
    if include_extended:
        ...
        for budget in (600., 800., 1000., 1100.):
            ...
            baseline_row = solve._candidate_row(tuple(baseline["local_xy_m"]), (0., 0.), 0., evaluator, early_stop=False)
            selected_row = solve._candidate_row(tuple(plan["selected"]["local_xy_m"]), (0., 0.), 0., evaluator, early_stop=False)
            budgets.append({"move_budget_m": budget, "selected": compact(selected_row),
                            "legacy_local_xy_m": baseline["local_xy_m"],
                            "legacy_same_geometric_bound_m": baseline_row["geometric_diameter_upper_bound_m"],
                            "legacy_same_capped_bound_m": baseline_row["diameter_upper_bound_m"]})
        ...
        for sides, step in ((180, .2), (360, .1), (720, .05)):
            ev = solve.ContinuousDiameterEvaluator(solve.first_region_outer_local((0, 0), 0, circle_sides=sides),
                                                   angle_step_deg=step, circle_sides=sides)
            sensitivity.append({"circle_sides": sides, "angle_step_deg": ev.step,
                                "fixed_candidate_bounds": [{"key": key, **compact(solve._candidate_row(q, (0., 0.), 0., ev, early_stop=False))} for key, q in fixed_points]})
        ...
        fine_evaluator = solve.ContinuousDiameterEvaluator(solve.first_region_outer_local((0, 0), 0, circle_sides=720),
                                                           angle_step_deg=.05, circle_sides=720)
        spatial_refinement = {"radial_step_m": 2.5, "azimuth_step_deg": .125,
                              "circle_sides": 720, "final_angle_step_deg": .05,
                              "selected": compact(solve._candidate_row(tuple(fine["selected"]["local_xy_m"]), (0., 0.), 0., fine_evaluator, early_stop=False)),
    ...
    result = {"data_provenance": "自建离线数学实验；不是官方数据、官方模拟器成绩或真实场地效果",
              "metric": "统一后验模型：首验、第二接收距离圆、第二扇区、5米有效投影及条带；M=360、h=0.1度且扇区/条带/投影同步增宽。评分C=min(A,B)，表中B均完整扫描。",
              "comparison_note": "同一模型和精度比较C才能评价选点变化；与旧模型数值之差还含几何约束变紧，均非实际点估计误差。提前停止等价性另见scoring_benchmark.json。",
...
def save_report(result: dict, path: Path) -> None:
    ...
    for scenario in result["scenarios"]:
        ...
        for row in scenario["rows"]:
            a, b = row["local_xy_m"]
            lines.append(f"| {row['label']} | ({a:.3f},{b:.3f}) | {row['diameter_upper_bound_m']:.6f} | {row['geometric_diameter_upper_bound_m']:.6f} | {row['move_and_detect_time_s']:.3f} | {'是' if row['guaranteed_reception'] else '否'} | {row['source_grid_received']}/{row['source_grid_count']} |")
        legacy = next(r for r in scenario["rows"] if r["key"] == "legacy_analytic")
        merged = next(r for r in scenario["rows"] if r["key"] == "merged")
        change = 1-merged["diameter_upper_bound_m"]/legacy["diameter_upper_bound_m"]
        lines += ["", f"在同一上界指标下，融合点相对原仓点下降 {change:.2%}。这表示所选策略获得更好的最坏直径保证；不能解释为真实定位误差同比下降。", ""]
    if result["budgets"]:
        lines += ["## 移动预算折中", "", "移动时间仅为辅助报告；默认优先最小直径上界，精度平局时才优先短路程。以下预算变化属于不同约束下的实验。", "", "| 预算/m | 融合点局部坐标/m | 原仓点的同口径C/m | 融合C/m | 移动+检测/s |", "|---:|---|---:|---:|---:|"]
        for row in result["budgets"]:
            s = row["selected"]
            a, b = s["local_xy_m"]
            lines.append(f"| {row['move_budget_m']:.0f} | ({a:.3f},{b:.3f}) | {row['legacy_same_capped_bound_m']:.6f} | {s['diameter_upper_bound_m']:.6f} | {s['move_and_detect_time_s']:.3f} |")
