...
from problem4.experiments.routing import LegacyPolicy,InterleavePolicy,DetectedStopPolicy
...
for cls in (LegacyPolicy,DetectedStopPolicy,InterleavePolicy):
 ...
 print(cls.__name__,summary,flush=True);body[cls.__name__]={'summary':summary,'episodes':rows}
(Path(__file__).resolve().parents[1]/'results'/'iterations_v2').joinpath(f'interleave_{split}_{count}.json').write_text(json.dumps(body,indent=2)+'\n',encoding='utf-8')
