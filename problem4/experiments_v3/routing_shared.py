"""Batch a common lateral baseline before committing to a source tour."""
import math
...
def get_class(name):
    ...
    attrs={'shared_initial_only':'initial' in tokens,'shared_min_channels':1 if 'one' in tokens else 2 if 'two' in tokens else 3,'shared_fresh_only':'all' not in tokens,'shared_detour_weight':0. if 'pure' in tokens else 1.,'shared_radius_options':'adaptive' in tokens,'shared_neighbor_first':'neighbor' in tokens,'shared_outer_only':'outer' in tokens,'shared_rounds':3 if 'staged' in tokens else 1,'shared_all_anchors':'staged' in tokens}
    if 'small' in tokens:attrs['shared_radius_m']=50.
    if 'path' in tokens:attrs['shared_detour_weight']=5.
    if 'large' in tokens:attrs['shared_radius_m']=150.
    if 'tiny' in tokens:attrs['shared_radius_m']=25.
    bases=(SharedBaselineMixin,EnrouteMixin,ProductionPolicy) if 'enroute' in tokens else (SharedBaselineMixin,ProductionPolicy)
